...
DynamicArray *dyaar_create_array(size_t elm_sz)
{
    ...
    self->capacity = elm_sz * 5;
    self->length = 0;
    self->array = malloc(elm_sz * 5);
    return self;
}
...

{
    ...
    //*((char *)self->array + len * self->elm_sz) = v;
    memcpy((char *)self->array + len * self->elm_sz ,v, self->elm_sz);
